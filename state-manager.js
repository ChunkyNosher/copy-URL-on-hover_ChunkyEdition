...
export class QuickTabStateManager {
  ...
  async save(tabs, cookieStoreId = null) {
    ...
    this.log(
      `Saving ${tabs.length} Quick Tabs for container: ${cookieStoreId}`,
    );

    try {
      // Load existing state for all containers
      const existingData =
        (await browser.storage.sync.get(this.stateKey)) || {};
      ...
      if (this.hasSessionStorage) {
        promises.push(
          browser.storage.session
            ...
            .then(() =>
              this.log(`Saved to session storage for ${cookieStoreId}`),
            )
            .catch((err) =>
  ...
  async load(cookieStoreId = null) {
    ...
    try {
      ...
      if (syncResult && syncResult[this.stateKey]) {
        ...
        if (this.hasSessionStorage) {
          await browser.storage.session
            ...
            .catch((err) =>
              console.error("Error populating session storage:", err),
            );
        }

        if (containerStates[cookieStoreId]) {
          this.log(
            `Loaded ${containerStates[cookieStoreId].tabs.length} tabs from sync storage`,
          );
          return containerStates[cookieStoreId];
        }
      }
      ...
      this.log(
        `No saved state found for ${cookieStoreId}, returning empty state`,
      );
      return { tabs: [], timestamp: Date.now() };
    } catch (err) {
      console.error(
        "[QuickTabStateManager] Error loading Quick Tab state:",
        err,
      );
      return { tabs: [], timestamp: Date.now() };
    }
  ...
  async addTab(tab, cookieStoreId = null) {
    ...
    if (existingIndex !== -1) {
      // Update existing tab
      currentState.tabs[existingIndex] = {
        ...currentState.tabs[existingIndex],
        ...tab,
      };
    } else {
  ...
  async clear(cookieStoreId = null) {
    ...
    this.log(
      `Clearing all Quick Tabs from storage for container: ${cookieStoreId}`,
    );
