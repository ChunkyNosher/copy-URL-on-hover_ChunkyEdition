...
✅ **Build:** Will need verification after integration  
✅ **ESLint:** 79 errors (36 in background.js), 11 warnings  
🔨 **Phase 3.1:** Handler classes created but NOT integrated yet
...
## 🎯 Your Mission: Complete Handler Integration

**Target:** Refactor background.js message handler from 628 lines (cc=93) to
<100 lines (cc<9)
...
**Current State:** Handlers exist but background.js still uses monolithic
listener
...
messageRouter.register(
  ['UPDATE_QUICK_TAB_SIZE', 'UPDATE_QUICK_TAB_SIZE_FINAL'],
  (msg, sender) => quickTabHandler.handleSizeUpdate(msg, sender)
);
...
messageRouter.register('openTab', (msg, sender) =>
  tabHandler.handleOpenTab(msg, sender)
);
messageRouter.register('saveQuickTabState', (msg, sender) =>
  tabHandler.handleSaveState(msg, sender)
);
messageRouter.register('getQuickTabState', (msg, sender) =>
  tabHandler.handleGetState(msg, sender)
);
...
- background.js reduced from 1795 lines → ~1200 lines (-595 lines)
- Message handler reduced from 628 lines → ~5 lines (-623 lines)
- Complexity reduced from cc=93 → cc<3
...
- [ ] ESLint errors reduced by ~20-25 (from 36 → ~10-15)
...
### ❌ DON'T Remove StateCoordinator

**Why:** StateCoordinator is already well-designed and functional. Keep it in
background.js.

### ❌ DON'T Change Message Format

**Why:** Content scripts expect specific message.action names. Only route, don't
modify.

### ❌ DON'T Test Without Building

**Why:** Handlers use ES6 modules. Must run `npm run build` first or imports
will fail.

---

## 📊 Reference: Handler Coverage
...
**Remaining Actions (if any):** Check background.js for unhandled message.action
strings
...
3. Update overall progress percentage (~47% → ~52%)
...
- Complexity: cc=93 → cc<9 (per handler)
- Lines: 628 → <10 (message listener)
- Maintainability: Testable, isolated handlers
- Extensibility: Add new handlers without touching routing

**Impact:**

- **Before:** Adding message handler = edit 1700-line file, risk breaking 20+
  actions
- **After:** Adding message handler = create 50-line handler class, register one
  line

**Next Milestone:**

Once Phase 3.1 complete, remaining background.js errors can be tackled
incrementally:

1. Phase 3.2: Extract initializeGlobalState complexity (cc=20 → cc<9)
...
1. ✅ **Build after every change** (ES6 modules need bundling)
2. ✅ **Test after every build** (no exceptions)
3. ✅ **Commit small chunks** (easy to revert if broken)
4. ✅ **Document integration issues** (helps next agent)

### When You're Done:

Create `v1.6.0-REFACTORING-PHASE3.2-NEXT-STEPS.md` if work continues.

**Good luck! This is the most impactful refactoring yet! 🚀**
...
2. Review MessageRouter.createListener() - returns function for
   chrome.runtime.onMessage
3. Verify handler methods are async (return promises)
4. Check browser console for detailed error messages
5. Don't hesitate to defer complex issues

**Remember:** Integration > Perfection. Get handlers working, document issues,
then optimize!
