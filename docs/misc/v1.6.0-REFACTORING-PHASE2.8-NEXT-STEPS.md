...
✅ **Build:** Successful  
⚠️ **ESLint:** 79 errors, 32 warnings
...
1. Removed `async` from `initMainFeatures()` in content.js (synchronous
   function)
...
- Warnings: 88 → 32 (-64% total reduction) 🎉
- Errors: 83 → 79 (-5% reduction)

---

## 🎯 Your Mission: Get to <30 Warnings

**Target:** Reduce 32 warnings → <30 warnings (-2 to -3 more)
...
- `tests/unit/storage/StorageAdapter.test.js:51,62` - async methods without
  await
...
**Strategy:** These are test utilities and mocks. They SHOULD be async (for API
compatibility), so:

1. **Option A:** Add `/* eslint-disable require-await */` comment above these
   functions
...
**Strategy:** Abstract base classes SHOULD have async methods (subclasses may
need await). Add suppression:
...
- [ ] Warnings: 32 → <25 (target achieved with margin)
...
### ❌ DON'T Touch background.js (51 errors)

**Why:** Requires dedicated 8+ hour session with handler registry pattern
refactoring.

**See:** `docs/misc/v1.6.0-REFACTORING-PHASE2.7-NEXT-STEPS.md` (lines 196-253)
for background.js strategy.

### ❌ DON'T Remove Async from Abstract Methods

**Why:** Subclasses may need async. Keep base class async for API consistency.

### ❌ DON'T Remove Async from Functions That Are Awaited
...
## 📊 Reference: Warning Breakdown (Current)

**By Category (Estimated):**

- `require-await` warnings: ~18 (mostly in tests/mocks/abstract classes)
- `max-lines-per-function`: ~2 (window.js:createTitlebar 157 lines, panel.js
  functions)
...
1. ✅ Fix easy wins first (unused vars, test suppression) → -10 to -15 warnings
2. ⏸️ Defer complexity issues (requires refactoring, not quick fixes)
3. ⏸️ Defer long functions (requires extraction, medium effort)
...
2. `docs/misc/v1.6.0-REFACTORING-MASTER-CHECKLIST.md` - Updated with Phase 2.7
   progress
3. `PHASE-2.6-COMPLETION-REPORT.md` - Pattern reference for systematic cleanup

**Background Context:**

1. `docs/misc/v1.6.0-REFACTORING-PHASE2.7-NEXT-STEPS.md` - Previous agent's
   handoff
...
- Warnings: 32 → <30 via test file suppression
...
- Warnings: 32 → <25 via systematic cleanup
...
# Example output: "✖ 111 problems (79 errors, 32 warnings)"
```
...
- **Target: <30 warnings (-66% cumulative)** ← You are here!
...
**Next Milestone:** Once <30 warnings achieved, focus shifts to:
...
2. ✅ **Small, focused commits** (easy to review)
3. ✅ **Stylistic only** (zero functional changes)
4. ✅ **Document clearly** (next agent will thank you)

### When You're Done:

Create `v1.6.0-REFACTORING-PHASE2.9-NEXT-STEPS.md` if work continues, OR
`PHASE-2.8-COMPLETION-REPORT.md` if you finish the warning reduction goal.
...
**Remember:** Progress > Perfection. Get to <30 warnings, document remaining
work, and move on!
