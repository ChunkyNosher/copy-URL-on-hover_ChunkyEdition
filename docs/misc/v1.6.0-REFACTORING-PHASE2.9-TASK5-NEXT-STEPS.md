...
Phase 2.9 Task 4 is **COMPLETE**. TitlebarBuilder has been successfully extracted from window.js, reducing the file from 918 → 647 lines (-271 lines, -30%). All 585 tests pass (added 44 new tests). ESLint warnings in window.js reduced from 2 → 1 (only render method remains).
...
- Overall ESLint: 83 → 85 problems (60 errors, 25 warnings) - slight increase from new file
...
2. **HoverDetectionPipeline** - Coordinating mouseover → handler → URL extraction
...
3. Create HoverDetectionPipeline (mouseover → handler selection → URL extraction)
