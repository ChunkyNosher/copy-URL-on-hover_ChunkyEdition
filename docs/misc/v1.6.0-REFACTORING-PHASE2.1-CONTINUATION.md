...
**Phase 2.1 Goal:** Decompose QuickTabsManager god object (1453 lines → ~400 lines facade)

**Current Progress:**

- ✅ **5 managers/handlers extracted** (StorageManager, BroadcastManager, StateManager, EventManager, CreateHandler)
- ✅ **1020 lines extracted** (70% of target reduction)
- ✅ **116 new tests added** (all passing, 100% coverage for extracted code)
- ⏳ **5 components remaining** (3 Handlers + 2 Coordinators)
- ⏳ **Facade integration** (refactor index.js to use all components)

---

## ✅ Completed Work This Session

### CreateHandler ✅ (170 lines, 13 tests) - **NEW**
...
**Complexity Reduction:** cc=25 → cc=3 (88% reduction)

**Quality Metrics:**
- ✅ ESLint: 0 errors
- ✅ Test Coverage: 100% (13 tests)
- ✅ Cyclomatic Complexity: <3 (target: <9)
...
export class VisibilityHandler {
  constructor(quickTabsMap, broadcastManager, storageManager, stateManager, minimizedManager, eventBus) {
    this.quickTabsMap = quickTabsMap;
    this.broadcastManager = broadcastManager;
    this.storageManager = storageManager;
  ...
  handleSoloToggle(quickTabId, newSoloedTabs) {
    ...
    // Emit event
    this.eventBus?.emit('tab:solo-toggled', { id: quickTabId, soloedOnTabs: newSoloedTabs });
  }
  ...
  handleMuteToggle(quickTabId, newMutedTabs) {
    ...
    // Emit event
    this.eventBus?.emit('tab:mute-toggled', { id: quickTabId, mutedOnTabs: newMutedTabs });
  }
...
**Responsibilities:**
- Close single Quick Tab - remove from map, minimized manager, broadcast, notify background
...
export class DestroyHandler {
  ...
  handleDestroy(id) {
    ...
    browser.runtime.sendMessage({
      ...
      id
    }).catch(err => console.error('[DestroyHandler] Failed to notify background:', err));
...
**Goal:** Reduce QuickTabsManager from 1453 lines → ~200 lines by delegating to extracted components
...
class QuickTabsManager {
  ...
  async detectContainerContext() { /* existing implementation */ }
  async detectCurrentTabId() { /* existing implementation */ }
  generateId() { /* existing implementation */ }
}
...
4. **Lint code** - ensure cc ≤ 9, depth ≤ 2
   ```bash
...
- [ ] Mean CC < 3.5 (currently all new code < 3) ✅
- [ ] Zero ESLint errors
...
- ❌ Change existing QuickTabsManager until ALL handlers/coordinators are extracted
- ❌ Break backward compatibility
- ❌ Remove any existing functionality
- ❌ Modify Phase 1 code (domain/storage layers are COMPLETE)
- ❌ Change test infrastructure (already perfect)
- ❌ Skip writing tests before implementation

### DO:

- ✅ Follow the extraction pattern used in CreateHandler
- ✅ Write tests FIRST before extracting each component (TDD)
- ✅ Use EventBus/EventEmitter for decoupled communication
...
- ✅ Use helper methods to reduce complexity (like CreateHandler)
- ✅ Follow import order conventions (ESLint will enforce)
...
```javascript
// Main method should be simple orchestrator (cc ≤ 3)
...
// Extract helper methods (each cc ≤ 3)
...
- [ ] All 10 components extracted (5/10 ✅ + 5/10 remaining)
...
3. **Phase 2.1 Previous Handoff:** `docs/misc/v1.6.0-REFACTORING-PHASE2.1-HANDOFF.md`
...
**Good luck with the remaining handlers and coordinators!** The hardest architectural work is done. This is now systematic extraction following the established pattern. You've got this! 🚀
...
- **UICoordinator** orchestrates: QuickTabWindow rendering, state → UI sync
- **SyncCoordinator** orchestrates: Storage ↔ state, broadcast → handlers
...
1. **Extract conditional logic** - if/else → separate methods
