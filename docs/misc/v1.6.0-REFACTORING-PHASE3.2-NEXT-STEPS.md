...
✅ **Build:** Successful with bundled background.js  
✅ **Phase 3.1:** MessageRouter integrated, handlers wired up 🔨 **Phase 3.2:**
Reduce remaining complexity in background.js

### Phase 3.1 Achievements (COMPLETE)

**Code Reduction:**

- background.js: 1795 → 954 lines (-841 lines, -47%)
- Message handler: 628 lines → 1 line (-627 lines, -99.8%)
- Removed 3 unused functions

**Quality Improvements:**

- ESLint errors in background.js: 36 → 20 (-16 errors, -44%)
- Total ESLint errors: 82 → 68 (-14 errors, -17%)
- Message routing complexity: cc=93 → cc<3 per handler

---

## 🎯 Your Mission: Reduce Remaining Complexity

**Target:** Fix remaining 20 ESLint errors in background.js by extracting
...
- Nested blocks: max-depth=5 (TARGET: max-depth≤2)
- Lines: 88 (TARGET: <70)
...
export class StorageFormatDetector {
  detect(data) {
    ...
    if (data && !Array.isArray(data.tabs) && !data.containers)
      return 'v1.5.8.14';
...
async function initializeGlobalState() {
  ...
  try {
    ...
    if (result.quick_tabs_state_v2) {
      ...
      if (migrator) {
        globalQuickTabState = migrator.migrate(
      ...
      console.log(
        '[Background] ✓ EAGER LOAD: No saved state found, starting with empty state'
      );
    }
...
- initializeGlobalState: cc=20 → cc<5
- max-depth: 5 → 2
- Lines: 88 → <40
...
- migrateQuickTabState: cc=10 → cc<6
- max-depth: 3 → 2
- ESLint errors: -2
...
- StateCoordinator.initialize: cc=15 → cc<6
- max-depth: 5 → 2
- ESLint errors: -5
...
Rollup is already configured to bundle background.js with all its imports. No
changes needed unless you encounter import resolution issues.
...
### ❌ DON'T Break Working Functions

**Why:** StateCoordinator is already functional. Only extract to reduce
complexity, don't change logic.

### ❌ DON'T Skip Testing After Each Extraction

**Why:** Each extraction should be tested independently. Build breaks are easier
to fix incrementally.

### ❌ DON'T Merge Strategy Pattern with Handler Pattern

**Why:** Handlers route messages. Strategies handle data formats. Keep concerns
separated.

---

## 📊 Reference: Remaining ESLint Errors
...
3. `docs/misc/v1.6.0-REFACTORING-PHASE3.1-NEXT-STEPS.md` - Previous phase
   context

**Background Context:**

1. `.github/copilot-instructions.md` - Robust Solutions Philosophy
2. `docs/manual/1.5.9 docs/copy-url-on-hover-refactoring-plan-v2-evidence-based.md` -
   Overall plan
...
3. Update overall progress percentage (~52% → ~55%)
...
- Complexity: cc=20 → cc<9 (per function)
- Nested blocks: max-depth=5 → max-depth≤2
- Maintainability: Isolated strategy classes vs monolithic functions
- Testability: Each strategy can be unit tested independently

**Impact:**

- **Before:** Modifying storage format = edit 100-line function, risk breaking
  all formats
- **After:** Modifying storage format = edit 30-line strategy class, other
  formats unaffected
...
1. ✅ **Build after every extraction** (catch import issues early)
2. ✅ **Test after every build** (no exceptions)
3. ✅ **Commit small chunks** (easy to revert if broken)
4. ✅ **Document strategy classes** (explain format differences)

### When You're Done:

Create `v1.6.0-REFACTORING-PHASE3.3-NEXT-STEPS.md` if work continues, or move to
Phase 4.

**Good luck! This phase eliminates the last complexity hotspots! 🚀**
...
**Remember:** Extraction > Perfection. Get strategies working, document issues,
then optimize!
