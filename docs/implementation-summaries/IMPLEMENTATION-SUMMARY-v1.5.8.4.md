...
Version 1.5.8.4 addresses a **critical bug** in v1.5.8.3 where URL detection failures blocked all keyboard shortcuts except "Copy Text". The bug was caused by three architectural issues:
...
- "Copy Text" could execute because `hoveredLink` was still truthy (from old hover)
...
5. **State management hardened** - explicit null handling (less undefined behavior)
