...
2. ✅ **"Clear Quick Tabs Storage" button now works**
   - Fixed error:
     `can't access property "then", browser.tabs.query(...) is undefined`
...
Changed the code to use a different variable name that doesn't shadow the
global:
...
Now it properly accesses Firefox's global `browser` object, and all API calls
work correctly.
...
1. **Test "Export Console Logs":**
   ...
   - **Expected:** File downloads successfully (like
...
- `docs/manual/BUGFIX-export-console-logs-and-clear-storage.md` - Technical
  documentation
