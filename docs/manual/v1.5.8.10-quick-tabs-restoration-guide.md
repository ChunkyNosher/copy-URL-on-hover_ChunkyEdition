...
**Purpose:** Fix regressions in v1.5.8.10 while maintaining current refactored
modular architecture  
**Repository:** ChunkyNosher/copy-URL-on-hover_ChunkyEdition  
**Target Version:** v1.5.8.10 → v1.5.9.0+  
...
This document provides comprehensive instructions for restoring Quick Tabs
functionality that regressed during the v1.5.8 → v1.5.8.10 refactoring.
**Critical:** All fixes must maintain the current modular structure in `src/`
directory.

### Core Issues to Fix

1. **Issue #35 & #51 Regression**: Quick Tab position/size NOT persisting across
   tabs
2. **Resize Functionality Broken**: Cannot resize Quick Tabs despite setting
   enabled
3. **Missing UI Elements**: Lost icon, webpage title, "Open in New Tab" button,
   pin button
4. **Quick Tab Manager Shortcut**: Move from Firefox shortcuts to extension
   settings menu
...
```javascript
// ✓ Had: Favicon + Title with webpage icon
...
// ✓ Had: Pin button with visual feedback
...
- ❌ No favicon rendering
- ❌ No webpage title display
- ❌ No "Open in New Tab" button
- ❌ No pin button functionality
- ❌ Resize handles not working (despite being "created")
- ❌ Position/size sync broken across tabs

**What's Present:**

- ✓ Modular class structure (`QuickTabWindow`)
- ✓ Basic titlebar with minimize/close buttons
- ✓ Drag functionality (but not integrated with sync)
- ✓ Resize handler code (but incomplete/broken)
...
**Root Cause:** The refactored `window.js` doesn't integrate with the state
management system.
...
function handleQuickTabPositionChange(quickTabId, left, top) {
  ...
  if (
    positionChangeThrottle[quickTabId] &&
    now - positionChangeThrottle[quickTabId] < 100
  ) {
    return;
  }
...
function handleQuickTabSizeChange(quickTabId, width, height) {
  ...
  if (
    sizeChangeThrottle[quickTabId] &&
    now - sizeChangeThrottle[quickTabId] < 100
  ) {
    return;
  }
...
**Problem:** The current `setupResizeHandlers()` only creates ONE handle (SE
corner). Need 8 handles like v1.5.8.
...
<div class="setting-row">
  <label class="setting-label">
    ...
    <span class="setting-hint"
      >Keyboard shortcut to toggle Quick Tab Manager panel</span
    >
  </label>
...
function stopShortcutRecording() {
  ...
  recordedKeys.clear();
  document
    .getElementById('quickTabManagerShortcut')
...
browser.storage.local.get('quickTabManagerShortcut').then(result => {
  if (result.quickTabManagerShortcut) {
    document.getElementById('quickTabManagerShortcut').value =
      result.quickTabManagerShortcut;
  } else {
...
browser.storage.onChanged.addListener((changes, area) => {
  if (area === 'local' && changes.quickTabManagerShortcut) {
    quickTabManagerShortcut =
      changes.quickTabManagerShortcut.newValue || 'Ctrl+Alt+Z';
  }
});
...
**Rationale:** Quick Tabs should ALWAYS persist across tabs. The toggle is
confusing and breaks expectations.

#### Step 5.1: Remove Setting from UI

**File:** `popup.html`  
**Action:** Find and DELETE this entire setting block (search for
...
<div class="setting-row">
  <label class="setting-label">
    <input
      type="checkbox"
      id="quickTabPersistAcrossTabs"
      class="setting-checkbox"
    />
    Persist Quick Tabs across browser tabs
    <span class="setting-hint"
      >Quick Tabs remain visible when switching tabs</span
    >
  </label>
</div>
```
...
export const DEFAULT_CONFIG = {
  ...
  // quickTabPersistAcrossTabs: true, // ❌ REMOVE THIS LINE
...
**Action:** Search for `CONFIG.quickTabPersistAcrossTabs` and remove conditional
checks
...
**Requirement:** Add option in Advanced tab to open settings in a separate HTML
page that syncs with popup.
...
<html lang="en">
  ...
  <body>
    <div class="page-header">
      <h1 class="page-title">🔗 Copy URL on Hover - Settings</h1>
...
- [ ] "Open in New Tab" button (🔗) visible and functional
...
- [ ] Switch to Tab 2 → Quick Tab appears in bottom-right
- [ ] Resize Quick Tab in Tab 2
- [ ] Switch to Tab 3 → Quick Tab has new size from Tab 2
- [ ] Move Quick Tab in Tab 3
- [ ] Switch back to Tab 1 → Quick Tab has new position from Tab 3

### Test 4: Pin Functionality

- [ ] Pin Quick Tab to Tab 1
- [ ] Switch to Tab 2 → Quick Tab NOT visible
- [ ] Switch back to Tab 1 → Quick Tab still visible
- [ ] Unpin Quick Tab in Tab 1
- [ ] Switch to Tab 2 → Quick Tab NOW visible
...
- [ ] Press `Ctrl+Shift+Q` on any page → Quick Tab Manager opens
- [ ] Press again → Manager closes

### Test 6: Full Settings Page

- [ ] Click "Open Full Settings Page" in Advanced tab
- [ ] New tab opens with full settings
- [ ] Change a setting in full page → Popup reflects change
- [ ] Change a setting in popup → Full page reflects change
- [ ] Close full page → Settings persisted
...
- ❌ Merge files back into monolithic structure
- ❌ Remove modular `src/` directory structure
- ❌ Break ES6 module imports/exports
- ❌ Remove existing abstraction layers

**DO:**

- ✅ Maintain separation: `features/`, `ui/`, `core/`, `utils/`
- ✅ Use existing helper functions in `utils/dom.js`
- ✅ Follow existing event-driven patterns
- ✅ Keep state management centralized
- ✅ Add new functions, don't replace entire files
...
3. **Runtime Messaging** (`browser.runtime.sendMessage`) - Background
   coordination
4. **Contextual Identities API** (`browser.contextualIdentities`) - Container
   support
...
- ✅ **Issue #35 RESOLVED**: Quick Tabs now persist across tabs
- ✅ **Issue #51 RESOLVED**: Position/size updates transfer between tabs
- ✅ **Resize Bug RESOLVED**: All 8 handles working correctly
- ✅ **UI Regression RESOLVED**: Full v1.5.8 feature parity
...
This guide provides GitHub Copilot Agent with everything needed to restore Quick
Tabs to full v1.5.8 functionality while preserving the modern refactored
architecture. All fixes are surgical - targeting specific methods and adding new
functions without breaking existing patterns.
