...
Quick Tabs created in one Wikipedia tab (Tab 1) completely disappear when
switching to another Wikipedia tab (Tab 2), instead of syncing their position,
size, and state as designed. The root cause is a **missing module import** in
the UICoordinator that prevents Quick Tabs from rendering when tabs become
visible after a cross-tab switch.
...
The `UICoordinator` class attempts to use a global function
`createQuickTabWindow` that is **not available in scope**:
...
export class UICoordinator {
  ...
  _createWindow(quickTab) {
    ...
    return createQuickTabWindow({
      // ❌ UNDEFINED - Function not imported!
      id: quickTab.id,
      url: quickTab.url
      // ...
    });
...
- Comment in code says "defined in window.js and attached to global scope" but
  this is **incorrect**
...
The `createQuickTabWindow` function from `src/features/quick-tabs/window.js` is
**never imported or made available** to UICoordinator. The
`/* global createQuickTabWindow */` comment is a JSLint directive that only
suppresses linting warnings - it doesn't actually import or define anything.
...
The **only** failing component is the UI rendering in
...
Issue #35 is caused by a **missing module import** in `UICoordinator.js`. The
function `createQuickTabWindow` is never imported, causing a `ReferenceError`
when the coordinator attempts to render Quick Tabs after a tab switch.

The cross-tab synchronization mechanism (storage.onChanged, state hydration,
event bus) works perfectly - the failure occurs **only** at the final rendering
step due to the missing dependency.
