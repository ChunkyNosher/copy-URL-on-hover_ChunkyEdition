...
**Objective:** Re-enable dual BroadcastChannel + browser.storage sync WITHOUT memory leaks  
...
This guide provides a complete, production-ready implementation for restoring the pre-v1.6.0 dual-channel sync system that enabled Quick Tabs to sync across ALL domains. The solution includes **four layers of memory leak protection** and additional enhancements to guarantee Scenarios 1 and 2 work as intended.
...
4. Added `loadFromCurrentContainer()` for cases where container isolation is needed
...
The `loadAll()` method now returns global Quick Tabs, so `handleTabVisible()` already works correctly. However, add logging to confirm:
...
**File:** `src/features/quick-tabs/managers/VisibilityManager.js` (or equivalent)
...
Quick Tabs are filtered by container, so only tabs in the same container see them.
...
**File:** `src/features/quick-tabs/coordinators/UICoordinator.js` (or equivalent)
...
**Purpose:** Provide additional cross-origin sync path via background script relay
...
**Memory Leak Protection:**
- [ ] Layer 1 (Rate Limiting): `_checkWriteRateLimit()` called in `_persistBroadcastMessage()`
- [ ] Layer 2 (Write Source Tracking): `writeSource` added to all storage writes
- [ ] Layer 3 (Deduplication): `_isDuplicateMessage()` implemented in `handleStorageChange()`
...
This implementation guide provides a complete, production-ready solution for restoring cross-domain Quick Tab sync while preventing the memory leaks that caused it to be disabled in v1.6.0.

**Key Innovations:**

1. **Four-Layer Protection:** Rate limiting + write tracking + deduplication + debouncing
2. **Global Visibility:** Container-agnostic rendering with Solo/Mute support
3. **Dual-Channel Sync:** BroadcastChannel (fast, same-origin) + Storage Events (reliable, cross-origin)
4. **Scenario Compliance:** Specifically designed to satisfy issue #47 requirements
