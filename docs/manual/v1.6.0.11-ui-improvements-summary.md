...
- ✅ Clear workflow: Toggle checkboxes → Press "Save Settings"
...
<label class="group-header">
  <span class="group-icon">▶</span>  ← Doesn't rotate!
  User Actions (6 categories)
</label>
```
...
<input type="checkbox" class="group-toggle" />
<label class="group-header">
  <span class="group-icon">▶</span>  ← Now rotates!
  User Actions (6 categories)
</label>
```
...
CSS (After):
.group-header {
  align-items: baseline;  ← Aligns with text baseline
}
...
- ✅ Consistent with typography best practices
- ✅ Buttons "sit" on the same line as text
...
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  ▼ User Actions        (6 categories)    [Select] [Deselect]│
│  └─┬──────────────────────┬──────────────────┬─────┬────────│
│    │                      │                  │     │        │
│    Triangle              Title             Count  Gap  Buttons│
│    Rotates               Text              Text   41px Aligned│
│    90° on                Baseline          Info        Baseline│
│    expand                                                    │
│                                                             │
│     □ 🔍 URL Detection                                      │
│     □ 👆 Hover Events                                       │
...
1. ✅ Triangle rotates when expanded (CSS `+` selector)
2. ✅ Buttons align with title text (baseline alignment)
3. ✅ 25px extra spacing between count and buttons
...
3. Press "Save Live Filters" button ← Separate action
4. Toggle Export filter checkboxes
5. Press "Save Export Filters" button ← Another separate action
6. Go to other tabs and change settings
7. Press "Save Settings" button ← Third save action!
...
5. Press "Save Settings" button ← Saves EVERYTHING
...
.group-toggle:checked + .group-header .group-icon {
  transform: rotate(90deg); /* ← No !important needed */
}
...
.group-count {
  margin-right: 41px; /* ← Increased from 16px */
}
```
...
async function saveFilterSettings(filterType) {
  const settings = {};
  document
    .querySelectorAll(`.category-checkbox[data-filter="${filterType}"]`)
...
2. **Better Visual Design**
   - Triangle icons now animate (rotate 90°) when expanding groups
   - Buttons properly aligned with text (not floating)
   - Better spacing makes UI feel less cramped

3. **Improved Workflow**
   - Toggle any settings → Press "Save Settings" → Done
...
```
Tab → "Save Live Filters" button
Tab → "Reset Live" button
Tab → "Save Export Filters" button
Tab → "Reset Export" button
Tab → "Save Settings" button (in footer)
```
...
- [x] Triangle rotates 90° when group expands
...
**Summary:** All UI improvements implemented successfully with better UX,
