...
When users attempted to create Quick Tabs or open the Quick Tabs Manager panel,
the following issues occurred:

1. **Quick Tabs Not Appearing:** Pressing the Q key on a link showed "Quick Tab
   Created" notification but no window appeared
...
All Quick Tab operations fell back to legacy creation path because the manager
...
1. **PanelUIBuilder Class Design:** All methods in `PanelUIBuilder` are defined
   as static methods:
...
3. **JavaScript Behavior:** In JavaScript, static methods CANNOT be called on
   instances:
   ```javascript
   class Test {
     static staticMethod() {
...
The issue was introduced during the Phase 2.10 refactoring when `PanelUIBuilder`
was extracted from `panel.js`. All methods were correctly made static (no
instance state needed), but some call sites weren't updated to use static
invocation.
...
**Note:** Also fixed the API usage - `renderContainerSection` returns an element
that needs to be appended, not mutate the container directly.
...
- **Original Bug Report:** Console log in
  `docs/extension-consoleoutput-logs/console-export-2025-11-20_13-18-23.log`
- **Refactoring Context:** Phase 2.10 extraction documented in
  `docs/misc/v1.6.0-REFACTORING-PHASE*.md`
...
This fix resolves a critical initialization bug that prevented the Quick Tabs
feature from working. The root cause was incorrect invocation of static methods
as instance methods, which JavaScript does not support. By changing all call
sites to use proper static invocation (`PanelUIBuilder.method()` instead of
`this.uiBuilder.method()`), the initialization succeeds and all features work as
expected.

The fix is minimal (7 lines of actual code changes), surgical (no breaking
changes), and well-tested (100% test coverage maintained).
