...
This document specifies a production-grade architecture for Quick Tabs state management that prioritizes **correctness and reliability** over simplicity. The design handles 50-100+ Quick Tabs simultaneously with 100-200ms synchronization latency while ensuring state consistency across sidebar, background script, and content scripts.
...
- **Minimal Complexity:** Avoids over-engineering; each component serves a specific purpose
...
async function _persistToStorage() {
  ...
  try {
    ...
    if (!readBack[STORAGE_KEY] || readBack[STORAGE_KEY].checksum !== stateToWrite.checksum) {
      console.error('[Background] WRITE VALIDATION FAILED - data corruption detected');
...
Firefox's `storage.onChanged` fires events **in arbitrary order** and may fire **multiple times** for the same write. The sidebar must detect and ignore duplicates while processing legitimate state changes in correct order.
...
**Why this works:** Monotonically increasing revision ensures out-of-order events are rejected.
...
if (newState.checksum && newState.checksum !== _computeStateChecksum(newState.tabs)) {
...
**Why this works:** Multiple storage.onChanged events for same write are skipped.
...
async function _checkStorageHealth() {
  ...
  if (age > 5000) {
    console.warn('[Sidebar] Storage listener may be broken - no events for', age, 'ms');
...
**Recovery:** Request state directly via message if storage events stop arriving.
...
**Recovery:** Sidebar continues using storage.onChanged events (which fire even if background restarts). When background restarts, it writes current state to storage, triggering sidebar update.
...
1. **Stateless Messages (no persistent port):** Each update is independent, no streaming capability
2. **100-200ms Latency:** Not real-time, but acceptable for UI updates
3. **Storage I/O Bottleneck:** IndexedDB writes limited to ~20-50ms; can't go faster
4. **Firefox MV2 Only:** Chrome would need different background persistence strategy
...
3. **Recovers gracefully** (corruption detection, fallback messages, health probes)
