...
**Replacement:** Modern `runtime.sendMessage()` + `storage.onChanged` stateless architecture  
...
**The problem:** This incomplete migration created 20 documented issues including:
...
**The solution:** Complete the migration by removing all port-based code and standardizing on the proven `tabs.sendMessage()` pattern already working in `broadcast-manager.js`.
...
- The features currently handled by port will use `storage.onChanged` + timeout fallback

**Replacement pattern:**
Instead of port messages, use Promise-based messaging with explicit error handling and storage fallback.
...
**What it did:** Attempted to reconnect to background script when port disconnected, with exponential backoff up to 10 seconds. This is now handled by Promise timeout + retry.
...
**Why simplify:** Current implementation waits 10 seconds to establish port. With Promise-based messaging, we can just wait for tab ID with 2-3 second timeout. If it fails, proceed with graceful degradation.

**Key change:** Reduce from 10-33.5 seconds blocking to 2-3 seconds max with non-blocking initialization.
...
**What it did:** Queued messages while port was connecting. With stateless messaging, we just send the message and it either succeeds or we use storage as fallback.
...
**What needs fixing:**
- `RESTORE_DEDUP_WINDOW_MS` is coupled to `PORT_RECONNECT_MAX_DELAY_MS` (Issue #18)
- When we remove `PORT_RECONNECT_MAX_DELAY_MS`, this will break
- **Solution:** Replace with explicit constant (50ms is sufficient for local dedup)
...
**The problem (Issue #19):** Constant is 300ms but code uses 300 + 100 = 400ms. They don't match.
...
**Issue #15 problem (Promise chaining contamination):**
Catch block returns false instead of rejecting promise, breaking chain semantics.

**Issue #16 problem (Circuit breaker off-by-one):**
Increment happens before check, creating dead zone.
...
**What it did:** Maintained a registry of all port connections from content scripts, tracking which tab has which port open. With tabs.sendMessage(), we don't need this - we just send to the tab directly.
...
2. Remove all PORT_RECONNECT_* constants
3. Remove portReconnectState and related tracking
4. Keep port-related event handlers for now (use as template for Promise version)
...
- [ ] All PORT_RECONNECT_* constants removed
...
- **Files simplified:** 4 (content.js, storage-utils.js, MessageRouter.js, broadcast-manager.js)
- **Bundle size:** -50-80KB
...
3. **Verify Promise error handling:** New code uses try/catch with storage fallback.

4. **Test with slow connections:** Use 2G throttling to verify timeout handling.

5. **Check for coupling before removing:** Search all references before deleting constants.
