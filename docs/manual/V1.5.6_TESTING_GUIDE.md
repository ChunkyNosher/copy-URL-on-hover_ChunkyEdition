...
Version 1.5.6 introduces Pointer Events API for Quick Tab drag/resize operations
and fixes the slot number reset bug in debug mode. This guide provides
comprehensive testing procedures to validate all changes.
...
3. Move mouse **very quickly** across the screen (left to right, then up and
   down)
...
✅ Quick Tab follows cursor perfectly at all speeds  
✅ Quick Tab never "jumps" or "detaches" from cursor  
✅ Cursor remains "locked" to title bar throughout drag  
✅ Console shows `[POINTER MOVE]` messages during drag  
✅ Console shows `[POINTER UP] Drag ended` with final position

### What Changed

- **Before**: Mouse events could miss during fast movements, causing RAF to skip
  updates
...
✅ `[POINTER CANCEL]` fires before tab switch  
✅ Console shows emergency save message  
✅ Quick Tab in Tab 1 stays at dragged position (doesn't revert)  
✅ If Quick Tab appears in Tab 3, it's at the new position (not old position)  
✅ No "jump back" when switching back to Tab 1

### What Changed

- **Before**: `mouseup` never fired if tab switched, position lost
- **After**: `pointercancel` explicitly fires before tab switch, triggering
  emergency save
...
✅ First 3 Quick Tabs labeled "Slot 1", "Slot 2", "Slot 3"  
✅ Console shows `[SLOTS] Reset slot numbering - next Quick Tab will be Slot 1`
after Esc  
✅ New Quick Tab after Esc is labeled **"Slot 1"** (not "Slot 4")

### Steps (Alternative - Clear Storage Button)

1. Open 3 Quick Tabs
2. Click extension icon → Advanced tab → "Clear Quick Tabs Storage"
3. Open a new Quick Tab

### Expected Results

✅ Console shows slot reset message  
✅ New Quick Tab is "Slot 1" (not "Slot 4")

### What Changed

- **Before**: Slot numbers never reset, kept incrementing indefinitely
- **After**: `resetQuickTabSlots()` called in `closeAllQuickTabWindows()` and
...
**Objective**: Verify each Quick Tab maintains its slot number when switching
tabs

### Steps

1. Tab 1: Open Quick Tab A → labeled "Slot 1"
2. Tab 1: Open Quick Tab B → labeled "Slot 2"
3. Switch to Tab 2
...
✅ Quick Tab A always shows "Slot 1" in all tabs  
✅ Quick Tab B always shows "Slot 2" in all tabs  
✅ Slot numbers don't change when switching tabs  
✅ Slot numbers are tied to Quick Tab ID, not tab order
...
✅ Quick Tab appears in Tab 2 (youtube.com) within **<200ms**  
✅ Position in Tab 2 is **(500, 300)** (not the original 100, 100)  
✅ Console in Tab 2 shows
`[Runtime] Received UPDATE_QUICK_TAB_FROM_BACKGROUND`  
✅ Background.js console shows
`[Background] Updated Quick Tab ... position: (500, 300)`

### What Changed

- **Before**: Cross-domain sync took up to 10 minutes (storage.sync delay)
- **After**: browser.runtime messaging provides <100ms sync via background
  script
...
✅ No "escape" or "slip out" during fast resize  
✅ Minimum size constraints respected (300x200)  
✅ `[POINTER MOVE] Resizing` messages in console  
✅ If tab switched during resize, `[POINTER CANCEL] Resize cancelled` fires  
✅ Final size persists across tabs
...
- Enable touch emulation: F12 → Device Toolbar (Ctrl+Shift+M)
...
✅ Drag works with touch (same as mouse)  
✅ Resize works with touch  
✅ Console shows `e.pointerType === 'touch'` in debug logs (if implemented)  
✅ No additional code needed - Pointer Events handles it automatically
...
✅ Quick Tab 2 and 3 remain at original positions  
✅ Only Quick Tab 1 moves  
✅ Background.js shows only Quick Tab 1 update  
✅ `globalQuickTabState` in background maintains all 3 tabs
...
✅ Press Q while hovering over link → Quick Tab opens  
✅ Quick Tab displays correct URL  
✅ Quick Tab navigation controls work (back, forward, reload)
...
✅ Quick Tabs persist when switching tabs (same domain)  
✅ Quick Tabs persist when switching tabs (different domain)  
✅ Quick Tabs restore after browser restart (if storage.sync enabled)

### Storage

✅ "Clear Quick Tabs Storage" button works  
✅ Quick Tabs are removed from all tabs when cleared  
✅ Slot numbers reset after clearing
...
✅ Position updates appear smooth (60fps)  
✅ No memory leaks (check Task Manager)  
✅ CPU usage reasonable (<10% for extension process)
...
❌ **Before v1.5.6**: Quick Tabs jump to original position when switching tabs  
✅ **After v1.5.6**: Quick Tabs maintain position across tab switches

❌ **Before**: Position lost if tab switched during drag  
✅ **After**: Position saved via pointercancel emergency save

❌ **Before**: Drag slipping during fast movements  
✅ **After**: setPointerCapture prevents slipping

### Slot Number Bug (Should be FIXED)

❌ **Before v1.5.6**: Slot 4, 5, 6... after closing all tabs  
✅ **After v1.5.6**: Slot 1 after closing all tabs
...
✅ Pointer Events API supported since Firefox 38 (2015)  
✅ setPointerCapture supported since Firefox 59  
✅ All Pointer Events work correctly

### Zen Browser

✅ Based on Firefox, full Pointer Events support  
...
- ✅ Drag slipping eliminated (Test Case 1)
- ✅ Tab switch during drag handled (Test Case 2)
- ✅ Slot numbers reset on Esc/Clear Storage (Test Case 3)
- ✅ Slot numbers persistent across tabs (Test Case 4)
- ✅ Cross-domain sync <200ms (Test Case 5)
- ✅ All 8 resize handles work (Test Case 6)
- ✅ Touch input supported (Test Case 7)
- ✅ Multiple Quick Tabs independent (Test Case 8)
- ✅ No regressions (Test Case 9)
- ✅ Performance acceptable (Test Case 10)
