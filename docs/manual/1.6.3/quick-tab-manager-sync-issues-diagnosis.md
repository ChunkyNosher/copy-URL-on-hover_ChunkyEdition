...
The Quick Tab Manager sidebar panel fails to synchronize with actual Quick Tab state due to **incomplete storage persistence** after v1.6.3's removal of cross-tab Quick Tab functionality. Individual Quick Tab operations (close, minimize, restore) properly update local UI state and emit internal events, but these changes **never propagate to `browser.storage.local`**, causing the Manager to display stale data.

Additional issues include a **storage area mismatch** (Settings writes to `sync`, Manager reads from `local`) and **missing message handlers** for Manager-initiated actions.
...
**What's Missing:** No `browser.storage.local.set()` call after destruction. No storage change event logged.
...
**But:** There is **no listener** anywhere in the codebase that responds to `state:deleted` by saving state to storage. The event fires into the void.

**Architecture Gap:** When v1.6.3 removed cross-tab Quick Tab support, the storage persistence layer that previously listened to these events was removed, but no replacement was added for single-tab persistence.

### Required Fix

**Add a storage persistence handler** that listens for `state:deleted` events and saves the updated Quick Tab state to `browser.storage.local`. This handler should:
...
**Alternative approach:** Modify `DestroyHandler.handleDestroy()` to directly call a storage save method after cleanup, bypassing the event system.
...
**What's Missing:** No `browser.storage.local.set()` call after minimize. No storage change event.
...
**Why the indicator doesn't update:** The Manager's `renderQuickTabItem()` function (quick-tabs-manager.js lines 408-409) checks for `tab.minimized` or `tab.visibility?.minimized` from the **storage state**, not live page state. Since storage is never updated, the Manager never knows the tab was minimized.
...
2. Update storage with modified tab's `minimized: true` or `visibility.minimized: true` property
3. Ensure the saved state reflects the current `quickTabsMap` state including minimized tabs

**Dual fix needed:** The same handler must also respond to restore events (`handleRestore()` at lines 156-174) to update storage when tabs are restored.
...
**Critical detail:** First storage change is to `sync` area (empty), second is to `local` area (the actual state storage).
...
**Historical context:** Version v1.6.0.12 migrated Quick Tab state from `storage.sync` to `storage.local` due to quota errors (background.js comments at line 440-442). The Settings UI was never updated to match this change.

**Why it eventually clears:** The Manager eventually receives the correct storage change when content scripts clear their local state, but the initial Settings button action targets the wrong storage area.
...
2. Also notify background script to reset its cache:
   ...
   - This ensures background's `globalQuickTabState` cache is cleared immediately
...
### Log Evidence
When buttons are clicked, logs show Quick Tabs are **focused** instead of minimized/closed:
...
**But content script has no handlers registered** for these specific actions. Searching the entire codebase shows:
...
**Why errors are silent:** The Manager functions all have `.catch()` blocks (lines 717, 733, 751) that swallow rejected promises when no handler responds.

**Why tabs sometimes close anyway:** The logs show destroy events firing, suggesting clicks might be triggering click-through to the actual Quick Tab windows rather than properly sending messages. This is unreliable and depends on window positioning.
...
2. **For `MINIMIZE_QUICK_TAB`:**
   - Register handler that receives `{ action: 'MINIMIZE_QUICK_TAB', quickTabId }`
   - Call `VisibilityHandler.handleMinimize(quickTabId)`
   - After minimizing, save updated state to storage

3. **For `RESTORE_QUICK_TAB`:**
   - Register handler that receives `{ action: 'RESTORE_QUICK_TAB', quickTabId }`
   - Call `VisibilityHandler.handleRestore(quickTabId)` or `restoreById(quickTabId)`
   - After restoring, save updated state to storage

**Implementation location:** These handlers should be registered in the same message listener that handles `CLEAR_ALL_QUICK_TABS` (search for this in content.js or src/content.js).
...
**Architecture gap from v1.6.3 refactoring:** When cross-tab Quick Tab support was removed, the handlers were updated to only emit events locally:
...
The old cross-tab sync system included automatic storage writes after events. When it was removed, **no single-tab persistence layer was added to replace it**.
...
**Alternative lightweight fix:** Add direct storage writes to each handler method after state changes, bypassing the event system. This is simpler but less maintainable.
...
**Incomplete migration:** When v1.6.0.12 changed Quick Tab storage from `sync` to `local` (due to quota errors with large states), some UI components were not updated to match:
...
### Observed Behavior
- Background script logs: "Storage cleared (empty/missing tabs), clearing cache immediately"
...
**Problem:** The hash function was updated in v1.6.3.2 to include `saveId` to prevent collisions, but there's an edge case:

1. When storage is cleared to empty state: `{ tabs: [], saveId: '...', timestamp: ... }`
2. The hash is computed from: `{ saveId, tabData: [] }`
3. If two separate clear operations occur, they produce different `saveId` values
4. **But** if background's cache already has empty tabs, and storage is cleared again, the hash might not update properly due to the `_isSelfWrite()` check (line 1073-1082)
...
This is too aggressive - it skips the update even when `saveId` changed, which should indicate a new write operation.
...
**Code location to modify:** `_handleQuickTabStateChange()` function in `background.js` (lines 1068-1099).
...
**Current behavior:** `.catch()` blocks swallow errors (quick-tabs-manager.js lines 717, 733, 751)
...
**Recommendation:** Implement Option B as immediate fix, refactor to Option A in next major version.
...
The gap is the Storage Persistence Layer - no component currently fills this role for individual operations.

---

## References

- **Mozilla WebExtensions Storage API:** https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/API/storage
- **storage.onChanged documentation:** https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/API/storage/onChanged
- **runtime.sendMessage() documentation:** https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/API/runtime/sendMessage
...
- **v1.6.0.12** - Migrated from storage.sync to storage.local (incomplete migration)
