...
- **Enhanced Error Serialization**: Error objects are now serialized with ALL
  properties including non-enumerable ones like `stack`, `fileName`,
...
**Why:** These errors are logged by Firefox's internal C++/Rust code directly to
the Browser Console and cannot be intercepted by JavaScript.
...
2. **Enhanced Error Serialization**
   ...
   - Preserves non-enumerable properties (stack, fileName, lineNumber,
     columnNumber)
...
- **Overflow behavior**: Oldest entries are automatically removed when buffer is
  full
- **Memory usage**: Approximately 50-100 bytes per log entry (depending on
  message size)
...
4. [GlobalEventHandlers.onerror - MDN](https://developer.mozilla.org/en-US/docs/Web/API/GlobalEventHandlers/onerror) -
...
The console log export feature in v1.6.0.7 provides **comprehensive diagnostic
capability** for extension debugging. The enhanced error serialization ensures
that stack traces and error details are fully preserved, while the extensive
logging coverage captures every significant user action and system operation.
...
These enhancements make the exported logs significantly more useful for
