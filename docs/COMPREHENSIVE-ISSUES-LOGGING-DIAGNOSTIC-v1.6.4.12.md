...
Background.js state handlers use comment pattern throughout indicating deliberate design decision to skip Manager broadcasts. This pattern indicates "Updating cache only (no broadcast)" with rationale "Tabs sync independently via storage.onChanged". This conflicts with Manager's expectation of real-time updates based on its three-tier listener setup.
...
Search results show these functions are imported and used by content scripts, but zero imports in background.js.
...
Recommended structured logging format includes: timestamp, layer/function, status, and operational details.
...
Current flow shows background updating state, writing to storage, but not sending confirmations. Manager must wait for storage.onChanged event or polls every 10 seconds.

Should flow through: Background updates state, writes to storage.local, broadcasts confirmation via BroadcastChannel or PORT, Manager receives confirmation and re-renders immediately, with storage.onChanged and polling as fallback only.
...
- BroadcastChannel listener exists but callback may not handle all message types from background
