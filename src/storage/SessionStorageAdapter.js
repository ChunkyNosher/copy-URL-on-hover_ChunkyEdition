...
export class SessionStorageAdapter extends StorageAdapter {
  ...
  async save(containerId, tabs) {
    ...
    try {
      ...
      console.log(`[SessionStorageAdapter] Saved ${tabs.length} tabs for container ${containerId} (saveId: ${saveId})`);
      return saveId;
  ...
  async delete(containerId, quickTabId) {
    ...
    if (filteredTabs.length === containerData.tabs.length) {
      console.warn(`[SessionStorageAdapter] Quick Tab ${quickTabId} not found in container ${containerId}`);
      return;
    }
    ...
    console.log(`[SessionStorageAdapter] Deleted Quick Tab ${quickTabId} from container ${containerId}`);
  }
