...
- **Cross-tab sync via storage.onChanged + BroadcastChannel + Background-as-Coordinator**
...
- **browser.alarms API** - Scheduled tasks (`cleanup-orphaned`, `sync-session-state`, `diagnostic-snapshot`)
...
- **DOM Reconciliation** - Sidebar animation optimization with `_itemElements` Map
...
**Core Modules:** QuickTabStateMachine, QuickTabMediator, MapTransactionManager, TabStateManager, BroadcastChannelManager, QuickTabGroupManager, NotificationManager
...
**Event Flow:** Port connection → Background writes storage → storage.onChanged + BroadcastChannel → `scheduleRender()` → hash check → renderUI
...
- Message types: `quick-tab-created`, `updated`, `deleted`, `minimized`, `restored`
...
**Key Exports:** `STATE_KEY`, `SESSION_STATE_KEY`, `logStorageRead()`, `logStorageWrite()`, `canCurrentTabModifyQuickTab()`, `validateOwnershipForWrite()`, `writeStateWithVerificationAndRetry()`

**Storage Keys:**
- `quick_tabs_state_v2` - Permanent Quick Tabs (storage.local)
- `session_quick_tabs` - Session Quick Tabs (storage.session, v1.6.3.7-v3)

**CRITICAL:** Use `storage.local` for permanent Quick Tab state, `storage.session` for session-scoped tabs.
...
- **v1.6.3.7-v3:** storage.session, BroadcastChannel, alarms, tabs.group(), DOM reconciliation, notifications
...
- **v1.6.3.7-v1:** Keepalive (20s), circuit breaker, debounced renderUI, differential storage updates
...
**BroadcastChannel (v1.6.3.7-v3):** `quick-tab-created`, `quick-tab-updated`, `quick-tab-deleted`, `quick-tab-minimized`, `quick-tab-restored`
